...
export default class Home extends GatedCommand {
  ...
  private async aiIntegrationMenu(): Promise<void> {
    ...
    this.log('')
    this.log(`  RIFT exposes ${bold('61 tools')} via MCP (Model Context Protocol).`)
    ...
    this.log('')
    this.log(`  ${bold('Available tools (61):')}`)
    this.log(`    ${dim('backtest, research, compare, sweep, smart_sweep,')}`)
    this.log(`    ${dim('walk_forward, montecarlo, quick_test, indicator_stats,')}`)
    this.log(`    ${dim('feature_importance, health, tearsheet, save_optimized,')}`)
    this.log(`    ${dim('workbench_create, workbench_update, workbench_show,')}`)
    this.log(`    ${dim('list_strategies, list_data, fetch_data, doctor, experiments,')}`)
    this.log(`    ${dim('algo_start, algo_status, algo_stop,')}`)
    this.log(`    ${dim('portfolio_start, portfolio_status, portfolio_stop, portfolio_alerts,')}`)
    this.log(`    ${dim('tca_report, pnl_attribution, generate_report, api_start,')}`)
    this.log(`    ${dim('var_report, audit_export, strategy_versions,')}`)
    this.log(`    ${dim('scout, manual_trade')}`)
    this.log('')
